...
| 对象 | 规范 | 示例 |
|------|------|------|
| 表名 | 小写 + 下划线，业务前缀 | `oopay_order`, `oopay_merchant` |
| 字段名 | 小写 + 下划线 | `create_time`, `update_time` |
...
| 币种 | 数据类型 | 说明 |
|------|----------|------|
| CNY (人民币) | `BIGINT` | 单位为**分**，避免浮点精度问题 |
| USDT | `DECIMAL(20,8)` | 8 位小数精度 |
| 其他法币 | `BIGINT` | 统一使用最小单位 |
...
| 手机号 | AES-256-GCM | 脱敏显示，加密存储 |
| API 密钥 | AES-256-GCM | 密钥分片存储 |
| 银行卡号 | AES-256-GCM | 掩码显示 |
| 身份证号 | AES-256-GCM | 掩码显示 |

---

## 2. 核心表结构

### 2.1 商户表 (oopay_merchant)

```sql
CREATE TABLE oopay_merchant (
...
### 2.2 商户通道配置表 (oopay_merchant_channel)

```sql
CREATE TABLE oopay_merchant_channel (
...
### 2.4 订单流水表 (oopay_order_log)

```sql
CREATE TABLE oopay_order_log (
    id                  BIGINT          NOT NULL AUTO_INCREMENT,
...
### 2.5 账户表 (oopay_account)

```sql
CREATE TABLE oopay_account (
    id                  BIGINT          NOT NULL AUTO_INCREMENT,
...
### 2.6 账户流水表 (oopay_account_flow)

```sql
CREATE TABLE oopay_account_flow (
    id                  BIGINT          NOT NULL AUTO_INCREMENT,
...
### 2.7 支付通道表 (oopay_channel)

```sql
CREATE TABLE oopay_channel (
    id                  BIGINT          NOT NULL AUTO_INCREMENT,
    channel_code        VARCHAR(32)     NOT NULL COMMENT '通道编码',
...
### 2.8 回调记录表 (oopay_notify_record)

```sql
CREATE TABLE oopay_notify_record (
    id                  BIGINT          NOT NULL AUTO_INCREMENT,
...
### 2.9 对账记录表 (oopay_reconcile)

```sql
CREATE TABLE oopay_reconcile (
    id                  BIGINT          NOT NULL AUTO_INCREMENT,
...
### 2.10 对账差异表 (oopay_reconcile_diff)

```sql
CREATE TABLE oopay_reconcile_diff (
    id                  BIGINT          NOT NULL AUTO_INCREMENT,
...
### 2.11 系统配置表 (oopay_config)

```sql
CREATE TABLE oopay_config (
    id                  BIGINT          NOT NULL AUTO_INCREMENT,
...
### 2.12 操作日志表 (oopay_operation_log)

```sql
CREATE TABLE oopay_operation_log (
    id                  BIGINT          NOT NULL AUTO_INCREMENT,
...
-- 订单表按时间范围分区（月分区）
-- 适合订单量 > 1000万/月的场景
ALTER TABLE oopay_order PARTITION BY RANGE (YEAR(create_time) * 100 + MONTH(create_time)) (
...
-- 账户流水表按时间范围分区
ALTER TABLE oopay_account_flow PARTITION BY RANGE (YEAR(create_time) * 100 + MONTH(create_time)) (
    PARTITION p202501 VALUES LESS THAN (202502),
    PARTITION p202502 VALUES LESS THAN (202503),
    PARTITION p_future VALUES LESS THAN MAXVALUE
);
